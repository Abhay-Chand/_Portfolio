    ...
    <section id="analytics" style={{ padding: 'clamp(5rem, 10vh, 8rem) clamp(1rem, 6vw, 5rem)', position: 'relative', zIndex: 1, maxWidth: '1200px', margin: '0 auto' }}>
      <motion.div initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: '-80px' }} transition={{ duration: 0.8 }}>
        <div style={{ textAlign: 'center', marginBottom: '3rem' }}>
          <div className="section-label">ANALYTICS COMMAND CENTER</div>
          <h2 className="section-title" style={{ fontSize: 'clamp(2rem, 5vw, 3rem)' }}>Data Analytics Showcase</h2>
          <p style={{ fontFamily: "'Inter', sans-serif", fontSize: '0.88rem', color: 'rgba(148,163,184,0.6)', marginTop: '0.75rem' }}>
            Interactive recreations of real project dashboards
          </p>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '1rem', marginTop: '1.5rem' }}>
          {[
            ...
            <div key={kpi.label} className="glow-card" style={{ padding: '1.1rem 1.25rem', transition: 'all 0.25s ease' }} onMouseEnter={(event) => { event.currentTarget.style.borderColor = `${kpi.color}55`; }} onMouseLeave={(event) => { event.currentTarget.style.borderColor = 'rgba(56,189,248,0.14)'; }}>
