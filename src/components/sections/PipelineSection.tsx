    ...
    <section style={{ padding: 'clamp(5rem, 10vh, 8rem) clamp(1rem, 6vw, 5rem)', position: 'relative', zIndex: 1 }}>
      <motion.div initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: '-80px' }} transition={{ duration: 0.8 }} style={{ maxWidth: '1100px', margin: '0 auto' }}>
        <div style={{ textAlign: 'center', marginBottom: '3rem' }}>
          <div className="section-label">DATA ARCHITECTURE</div>
          <h2 className="section-title" style={{ fontSize: 'clamp(2rem, 5vw, 3rem)' }}>Data Engineering Pipeline</h2>
          <p style={{ fontFamily: "'Inter', sans-serif", fontSize: '0.88rem', color: 'rgba(148,163,184,0.6)', marginTop: '0.75rem' }}>
            Watch data travel from raw sources to AI-powered insights
          </p>
        </div>

        <div className="glow-card" style={{ padding: '2.5rem 1rem', overflow: 'hidden' }}>
            ...
            {steps.map((step, index) => {
              ...
              return (
                <g key={step.id}>
                  ...
                  <text x={cx} y={cy + nodeRadius + 18} textAnchor="middle" fill={step.color} fontSize="9" fontFamily="'Orbitron', monospace" fontWeight="700">
                    {step.label}
                  </text>
                  <text x={cx} y={cy + nodeRadius + 30} textAnchor="middle" fill="rgba(148,163,184,0.5)" fontSize="7.5" fontFamily="'JetBrains Mono', monospace">
                    {step.sub}
                  </text>
                </g>
              );
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: '1rem', marginTop: '1.5rem' }}>
          {[
            ...
            <div key={stat.label} className="glow-card" style={{ padding: '1rem 1.25rem', textAlign: 'center' }}>
